...
import { getPhysicsOptPhases, PhysicsOptPhase, PhysicsOptMessage } from "./physicsOptPhases";
import { usePhysicsOptAnimation } from "./usePhysicsOptAnimation";

interface PhysicsOptShowcaseProps {
  locale: string;
}
...
function getAvatar(role: string, name: string) {
  ...
  if (role === "purple") return <Sparkles className="w-4 h-4" />; // Gemini
  if (role === "orange") return <Search className="w-4 h-4" />; // Perplexity
  if (role === "red") return <Brain className="w-4 h-4" />; // GPT-5.2
  if (role === "blue") return <Bot className="w-4 h-4" />; // Claude
  if (name.includes("Profiler")) return <Activity className="w-4 h-4" />;
  if (name.includes("Builder")) return <Cpu className="w-4 h-4" />;
  if (name.includes("Outcome")) return <Zap className="w-4 h-4" />;
  if (role === "system") return <Activity className="w-4 h-4" />;
  return <Bot className="w-4 h-4" />;
}

export default function PhysicsOptShowcase({ locale }: PhysicsOptShowcaseProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  // Get phases with locale
  const phases = useMemo(() => getPhysicsOptPhases(locale), [locale]);
  ...
  const {
    ...
    handleManualPrev,
    resetAnimation,
  } = usePhysicsOptAnimation({ phases, isExpanded });
    ...
    <div className="w-full max-w-5xl mx-auto">
      <div
        className={`relative overflow-hidden rounded-2xl border transition-all duration-700 ease-in-out ${
          isExpanded
            ? "border-orange-500/30 dark:border-amber-600/50 bg-slate-50 dark:bg-slate-950/80 shadow-2xl shadow-orange-900/20 dark:shadow-amber-900/40"
            : "border-slate-200 dark:border-amber-900/20 bg-white dark:bg-slate-900/40 hover:border-orange-500/20 dark:hover:border-amber-600/40"
        } backdrop-blur-xl`}
      >
        {/* Background Glow */}
        <div className="absolute inset-0 bg-gradient-to-br from-orange-600/5 dark:from-amber-600/5 via-transparent to-orange-600/5 dark:to-amber-600/5 pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              <div className="relative">
                <div className="absolute inset-0 bg-orange-500 dark:bg-amber-500 blur-xl opacity-20 group-hover:opacity-40 transition-opacity duration-500" />
                <div className="relative p-3 rounded-xl bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-amber-900/30 shadow-lg group-hover:border-orange-500/30 dark:group-hover:border-amber-500/50 transition-colors">
                  <Zap className="w-6 h-6 text-orange-600 dark:text-amber-500" />
                </div>
              </div>
              <div className="text-left">
                <div className="flex items-center gap-2 text-[10px] font-mono text-orange-600 dark:text-amber-500 mb-1 tracking-widest uppercase">
                  <span className="px-2 py-0.5 rounded-full bg-orange-500/10 dark:bg-amber-500/10 border border-orange-500/20 dark:border-amber-500/30">
                    Cycle #49
                  </span>
                  <span className="text-slate-400 dark:text-slate-700">|</span>
                  <span className="flex items-center gap-1 text-slate-500">
                    <Clock className="w-3 h-3" /> Dec 13, 2025
                  </span>
                </div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-orange-700 dark:group-hover:text-amber-400 transition-colors tracking-tight">
                  {locale === "cs"
                    ? "Multi-Agent: Physics Optimization"
                    : "Multi-Agent: Physics Optimization"}
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">
                  {locale === "cs"
                    ? "Claude + Gemini + Perplexity + GPT-5.2, jeden problém, 60 FPS."
                    : "Claude + Gemini + Perplexity + GPT-5.2, 1 Problem, 60 FPS."}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 text-sm font-medium text-slate-600 dark:text-slate-500 group-hover:text-slate-900 dark:group-hover:text-white transition-colors relative">
              {!isExpanded && (
                <div className="hidden md:block absolute -left-12 top-1/2 -translate-y-1/2">
                  <ChevronRight className="w-5 h-5 text-orange-600 dark:text-amber-500 animate-[slide-right_2s_ease-in-out_infinite]" />
                </div>
              )}
              <span
                className={`hidden md:inline tracking-wide text-xs uppercase ${
                  !isExpanded
                    ? "text-orange-600 dark:text-amber-400 font-bold"
                    : ""
                }`}
              >
                {isExpanded
              ...
              <div
                className={`p-2 rounded-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 group-hover:bg-orange-500/10 dark:group-hover:bg-amber-500/10 group-hover:border-orange-500/30 dark:group-hover:border-amber-500/30 transition-all duration-300 ${
                  !isExpanded &&
                  "animate-pulse ring-2 ring-orange-500/20 dark:ring-amber-500/20"
                }`}
              >
                {isExpanded ? (
                  <ChevronUp className="w-4 h-4" />
                ) : (
                  <Play className="w-4 h-4 fill-current text-orange-600 dark:text-amber-500" />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-slate-200 dark:border-white/5 bg-white/50 dark:bg-slate-950/30 relative">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-slate-900/50 backdrop-blur-md sticky top-0 z-30 overflow-x-auto">
              {phases.map((phase, index) => (
                <div
                  ...
                  className={`flex items-center gap-2 cursor-pointer transition-all duration-300 whitespace-nowrap px-3 py-1.5 rounded-full ${
                    currentPhase === index
                      ? "bg-orange-100 dark:bg-amber-500/20 text-orange-700 dark:text-amber-300 border border-orange-200 dark:border-amber-500/30"
                      : currentPhase > index
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                      currentPhase === index
                        ? "bg-orange-600 dark:bg-amber-600 text-white dark:shadow-[0_0_10px_rgba(245,158,11,0.5)]"
                        : currentPhase > index
              ...
              {isAutoPlaying && !isUserPaused && (
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-orange-100 dark:bg-amber-500/10 border border-orange-200 dark:border-amber-500/20 text-orange-600 dark:text-amber-400 text-[10px] font-mono animate-pulse">
            ...
            >
              ...
              <div className="text-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 text-xs font-mono text-slate-500 dark:text-slate-400 mb-2">
                  <span className="w-2 h-2 rounded-full bg-orange-500 dark:bg-amber-500 animate-pulse" />
                      ...
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border backdrop-blur-md shadow-lg transition-all duration-300 hover:scale-110 ${
                          msg.role === "blue"
                            ...
                            : msg.role === "purple"
                            ? "bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-300 border-purple-200 dark:border-purple-400/30"
                            : msg.role === "orange"
                            ? "bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-300 border-orange-200 dark:border-orange-400/30"
                            : msg.role === "red"
                      ...
                      <div
                        className={`group relative p-5 rounded-2xl text-sm leading-relaxed border backdrop-blur-sm transition-all duration-300 hover:shadow-lg inline-block text-left ${
                          msg.role === "blue"
                            ...
                            : msg.role === "purple"
                            ? "bg-purple-50 dark:bg-purple-500/10 border-purple-100 dark:border-purple-400/30 hover:border-purple-300 dark:hover:border-purple-400/50 text-purple-900 dark:text-purple-100 shadow-[0_0_15px_rgba(168,85,247,0.15)]"
                            : msg.role === "orange"
                            ? "bg-orange-50 dark:bg-orange-500/10 border-orange-100 dark:border-orange-400/30 hover:border-orange-300 dark:hover:border-orange-400/50 text-orange-900 dark:text-orange-100 shadow-[0_0_15px_rgba(249,115,22,0.15)]"
                            : msg.role === "red"
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/90 dark:bg-slate-950/90 border-t border-slate-200 dark:border-white/5 backdrop-blur-xl flex items-center justify-between z-20">
              ...
              {currentPhase < phases.length - 1 ? (
                <Button
                  ...
                  disabled={isResuming}
                  className={`bg-gradient-to-r from-orange-600 via-amber-500 to-orange-700 dark:from-amber-700 dark:via-orange-600 dark:to-amber-800 hover:opacity-90 dark:shadow-[0_0_15px_rgba(245,158,11,0.4)] text-white gap-2 px-4 transition-all duration-300 ${
