# Claude Code Mastery: Advanced AI-Assisted Development 🚀
...
**Prerequisites:** This lesson assumes you have the Claude Code CLI installed and configured. If you haven't set it up yet, install it via `npm install -g @anthropic-ai/claude-code` or visit [Claude Code docs](https://docs.anthropic.com/en/docs/claude-code).
</Callout>

<VideoSwitcher alternatives={[{"id":"SUysp3sJHbA","title":"Claude Code Tutorial #1 - Introduction & Setup (Net Ninja)"},{"id":"kZ-zzHVUrO4","title":"How I use Claude Code for real engineering (Matt Pocock)"},{"id":"nGhsgdQplHw","title":"Claude Code 2.0 Agentic Coding (IndyDevDan)"},{"id":"wHHNtetJV0c","title":"Claude Code Plugins & MCP Toggles (JeredBlu)"},{"id":"D0Rgtgtzlts","title":"AI AGENTI - AI Programování (CZ)"}]} />
...
<ConceptCard title="Agentic vs. Assistive AI" icon="🤖">

**Traditional AI Assistants:** You ask → AI responds → You copy/paste → You debug

**Agentic AI (Claude Code):** You describe intent → AI plans → AI executes → AI verifies → You review
...
├── Code Reviewer Agent (quality checks)
└── Test Runner Agent (verification)
```
...
## 🔬 Lab 1: Configure Your First Hook
...
**💡 Aha Moment:** "I never have to think about formatting again. The hook handles it automatically, every single time!"
...
```text
❌ Bad: "I'm getting an error when I run the tests"

✅ Good: [paste full error stack trace]

✅ Better: Run npm test and the error will appear in your context
```
...
## 🔬 Lab 2: Multi-File Feature Implementation
...
**💡 Aha Moment:** "One prompt triggered coordinated changes across 4+ files, with proper ordering and verification. This would take me 30 minutes manually!"
...
**💡 Aha Moment:** "Claude Code didn't just suggest a fix - it systematically investigated, identified the root cause, and verified the solution. That's agentic debugging!"
...
<ConceptCard title="Holocron: Claude Code Mastery" icon="💎">
...
* **Debug workflow:** Show error → Investigate → Fix → Verify
