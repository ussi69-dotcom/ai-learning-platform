...
<ConceptCard title="The Knowledge Problem" icon="🧠">
...
## 🔬 Lab 1: Create Your First Embeddings
...
**💡 Aha Moment:** "This is exactly what embedding models do mathematically! They convert sentences into vectors where similar meanings = close vectors."
...
|--------|---------|-------------|
| **Cosine Similarity** | cos(θ) | Most common, normalized |
| **Euclidean Distance** | √Σ(a-b)² | Raw distance |
...
<Callout type="info">
**💰 Cost Considerations:** RAG systems have multiple cost components:
- **Embedding API calls:** ~$0.02-0.13 per million tokens (indexing + queries)
- **Vector database:** Free (self-hosted) to $70+/month (managed cloud)
- **LLM generation:** $1-15 per million tokens depending on model
- **Storage:** Vector databases need ~4KB per chunk (1536 dimensions × float32)

Start with free tiers (Chroma local, OpenAI's small model) for prototyping!
</Callout>
...
```
Pinecone:  🏢 Enterprise   | ⚡ Fastest   | 💰 $$
Chroma:    🧪 Prototyping  | 🐍 Pythonic  | 💰 Free
Weaviate:  🔗 Hybrid       | 📊 GraphQL   | 💰 Free/$
Qdrant:    🦀 Performance  | 🔧 Self-host | 💰 Free
```
...
**💡 Aha Moment:** "The AI went from guessing to giving precise, sourced answers. That's the power of RAG - grounding AI in real data!"
...
**💡 Aha Moment:** "RAG failures are usually systematic, not random. The debug process: Check chunks → Check retrieval → Check prompt → Check model."
...
```
□ Data Pipeline
  □ Document loader handles all your file types
  □ Chunking strategy tested with sample queries
  □ Embedding model appropriate for domain

□ Retrieval
  □ Vector database chosen and configured
  □ Top-K tuned for your use case
  □ Hybrid search enabled if needed

□ Generation
  □ Grounding instructions in system prompt
  □ Source citations in responses
  □ Fallback for "no relevant context"

□ Evaluation
  □ Retrieval accuracy measured (Recall@K)
  □ End-to-end answer quality tested
  □ Hallucination detection in place
```

---

<ConceptCard title="Holocron: RAG Fundamentals" icon="💎">
...
* **The Pipeline:** Index (chunk + embed) → Retrieve (similarity search) → Generate (grounded response)
