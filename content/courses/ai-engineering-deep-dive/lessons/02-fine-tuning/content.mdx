...
> **OpenAI charges about $5 to fine-tune GPT-4.1 mini per 1 million training tokens. But here's the secret: most projects don't need it.**
...
## 🔬 Lab 1: Build Your Training Dataset
...
**💡 Aha Moment:** "A single misplaced comma can corrupt your entire dataset. JSONL validation is non-negotiable!"
...
job = client.fine_tuning.jobs.create(
    ...
    model="gpt-4.1-mini",  # Or "gpt-4.1" for higher quality
...
completion = client.chat.completions.create(
    model="ft:gpt-4.1-mini:your-org:custom:abc123",  # Your model name
    messages=[
...
### GPT-4.1 mini Pricing (as of 2025)
...
Compare to base model pricing on the OpenAI pricing page. **Fine-tuned models usually carry a premium.**

---

## 🔬 Lab 2: Cost Calculator Challenge
...
- Training data: 2,000 examples × 500 tokens each
- Epochs: 3
...
```
Dataset tokens = 2,000 × 500 = 1,000,000 tokens
Training tokens = 1,000,000 × 3 epochs = 3,000,000 tokens
```

### Step 2: Calculate Training Cost
```
Training Cost = (3,000,000 / 1,000,000) × $5 = $15.00
```

### Step 3: Calculate Daily Usage
```
Daily Input = 5,000 × 150 = 750,000 tokens
Daily Output = 5,000 × 300 = 1,500,000 tokens

Daily Input Cost = (0.75M / 1M) × $0.80 = $0.60
Daily Output Cost = (1.5M / 1M) × $3.20 = $4.80
Daily Total = $5.40
```

### Step 4: Calculate Monthly Total
```
Monthly Usage = $5.40 × 30 = $162.00
Total First Month = $15 + $162.00 = $177.00
```

</Steps>

**💡 Aha Moment:** "$135.75/month for a specialized, reliable chatbot that delights customers? For many businesses, that's a no-brainer ROI!"
...
<ConceptCard title="Fine-Tuning Mastery" icon="💎">
...
* **The process:** Upload File → Create Job → Monitor → Use New Model
...
Need quick experiment? → Prompt Engineering
```
